...
ROT_GOAL_TOL = .1  # rad, tolerance to consider a goal complete
...
PATH_NAME = 'shortest_path_myhal1.npy'  # saved path from l2_planning.py, should be in the same directory as this file
...
class PathFollower():
    ...
    def follow_path(self):
        while not rospy.is_shutdown():
            ...
            # Adjust the control horizon dynamically as the robot approaches the goal  
            dist_to_goal = np.linalg.norm(self.pose_in_map_np[:2] - self.cur_goal[:2])
            min_time_trans = dist_to_goal / self.path_planner.vel_max  # Estimated time for translational movement  

            # Check if rotational adjustment is needed based on proximity to goal  
            if dist_to_goal < MIN_TRANS_DIST_TO_USE_ROT:
                angle_diff = np.abs(self.pose_in_map_np[2] - self.cur_goal[2])
                rot_error = min(2 * np.pi - angle_diff, angle_diff)  # Smallest rotational difference  
                min_time_rot = np.abs(rot_error) / 1.82  # Estimated time for rotation  
            else:
                min_time_rot = 0  

            # Compute and constrain control horizon  
            control_horizon = min_time_trans + min_time_rot  
            control_horizon = np.clip(control_horizon, CONTROL_HORIZON)

            # Generate trajectory rollout
            pose_traj = self.path_planner.trajectory_rollout(
                initial_pose=self.pose_in_map_np,
                lin_vels=self.all_opts[:, 0:1],
                rot_vels=self.all_opts[:, 1:2],
            )

            # Perform collision checking
            valid_pose_traj, last_valid_substep, last_valid_poses = self.path_planner.check_collisions(pose_traj)

            # Mask for valid trajectories (trajectories that reach their last substep without collision)
            valid_pose_traj_mask = ~np.isnan(valid_pose_traj[:, -1, :]).any(axis=1)

            # Select only valid trajectories
            valid_pose_traj = valid_pose_traj[valid_pose_traj_mask]
            
            if not (valid_pose_traj.shape[0] == 0):
                # Compute translational error and cost  
                trans_error = valid_pose_traj[:, -1, :2] - self.cur_goal[:2].reshape(1, 2)
                trans_cost = np.linalg.norm(trans_error, axis=1)

                # Compute rotational cost if the robot is close enough to require rotation  
                if not (dist_to_goal < MIN_TRANS_DIST_TO_USE_ROT):
                    rot_cost = 0
                else:
                    angle_diff = np.abs(valid_pose_traj[:, -1, 2] - self.cur_goal[2])
                    rot_error = np.minimum(2 * np.pi - angle_diff, angle_diff)
                    rot_cost = np.abs(rot_error)        

                # Compute final cost and select the best velocity option  
                final_cost = trans_cost + ROT_DIST_MULT * rot_cost
                best_idx = np.argmin(final_cost)
                
                # Assign the best control and trajectory  
                control = self.all_opts[valid_pose_traj_mask][best_idx]
                best_trajectory = valid_pose_traj[best_idx]

                # Publish the local path  
                self.local_path_pub.publish(utils.se2_pose_list_to_path(best_trajectory, 'map'))
            else:
                control = [-0.1, 0]
